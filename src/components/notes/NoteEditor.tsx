...
const MenuBar = ({ editor, onToggleFullscreen, isFullscreen }: { editor: Editor, onToggleFullscreen?: () => void, isFullscreen?: boolean }) => {
  ...
  const ToolbarButton = ({
    ...
    <Button
      ...
      disabled={disabled}
      className={cn(
        "h-8 w-8 p-0 transition-all duration-150 rounded-[8px]",
        disabled
          ? "opacity-40 cursor-not-allowed text-[var(--text-muted)]"
          : active
            ? "bg-[var(--brand-gradient)] text-white hover:brightness-105 shadow-sm"
  ...
  return (
    <div className="border-b border-[var(--border-default)] bg-[var(--bg-base)] sticky top-0 z-20 shadow-sm p-2 flex flex-wrap items-center gap-1.5 rounded-t-[12px]">
      {/* Undo/Redo Group */}
      ...
      <div className="flex items-center gap-0.5">
        ...
        <ToolbarButton
          ...
          active={editor.isActive('paragraph') && !editor.isActive('heading') && !editor.isActive('bulletList') && !editor.isActive('orderedList') && !editor.isActive('taskList') && !editor.isActive('blockquote')}
          label="Paragraph"
        />
      </div>
      ...
      {onToggleFullscreen && (
        <>
          ...
          <Button
            ...
            className="h-8 w-8 p-0 rounded-[8px] text-[var(--text-secondary)] hover:bg-[var(--bg-card-hover)] hover:text-[var(--text-primary)] transition-all shrink-0"
...
const NoteEditor = ({ content, onChange, editable = true, currentUserName, isFullscreen, onToggleFullscreen, onFocusChange }: NoteEditorProps) => {
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: 'prose prose-sm sm:prose-base dark:prose-invert max-w-[720px] mx-auto min-h-[500px] px-8 py-12 focus:outline-none text-[16px] leading-[1.7]',
      },
    },
  });
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--bg-card)] rounded-[16px] border border-[var(--border-default)] overflow-hidden shadow-sm">
